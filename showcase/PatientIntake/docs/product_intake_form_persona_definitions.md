...
2. **Primary Goals**
   - Accurately enter required fields with validation feedback to reduce data entry errors (<1 % error rate – KPI‑04).
...
*All functional references map to existing asset IDs:* FR‑001 (Secure demographic capture), FR‑002 (Insurance verification), FR‑003 (Immutable audit logging), FR‑005 (PDF generation with watermark), NFR‑001 (Encryption at rest & transit), NFR‑003 (Audit log integrity), KPI‑01 (Form submission response time <200 ms), KPI‑04 (Data entry error rate <1 %).
...
|-------------|--------------------------|-----|----------------|
| US‑001 | FR‑001 | KPI‑04 (PDF export security compliance) | RISK‑01 (Unauthorized data exposure) |
| US‑002 | FR‑002 | KPI‑05 (Test coverage targets) | RISK‑02 (Open‑source component vulnerabilities) |
| US‑003 | FR‑003 | KPI‑03 (Successful audit log generation) | RISK‑03 (Deployment misconfiguration) |
| US‑004 | FR‑004 | KPI‑02 (System availability) | RISK‑01 (Unauthorized data exposure) |