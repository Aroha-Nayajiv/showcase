...
### 1.1. Tripartite Ecosystem: Donor, Beneficiary, Merchant
...
To resolve KYC/AML compliance gaps and ensure the integrity of the tripartite ecosystem, the legal structure of the NGO Operator is a critical governance component.

**Entity Classification:** NGO Operators must be registered as 501(c)(3) non-profit organizations or their international equivalents, ensuring they are legally bound to fiduciary duties regarding beneficiary data and funds.
**KYC/AML Obligations:** The NGO Operator is responsible for performing Know Your Customer (KYC) and Anti-Money Laundering (AML) checks on Beneficiaries before onboarding them to the platform. This includes verifying identity documents and ensuring beneficiaries are not on any sanctions lists.
**Data Stewardship:** The NGO Operator acts as the primary data steward for Beneficiary PII. They must maintain secure, encrypted records of eligibility criteria and demographic data, which are then hashed and transmitted to the Platform Administrator for anonymized ledger operations.
**Compliance Auditing:** The NGO Operator must submit to annual compliance audits by the Platform Administrator to ensure adherence to FTC Anonymity Guidelines and local data residency laws.

### 1.3. Capability Mapping & Interaction Matrix

The following matrix maps each actor role to the core capabilities they interact with, ensuring that the operating model supports the required business functions.

| Actor Role | Primary Capabilities | Key Constraints & Concerns |
| :--- | :--- | :--- |
| Donor ([ACT-80C62C7814](../project_glossary.md#act-80c62c7814)) | CAP-IDENTITY-ACCESS-MANAGEMENT, [CAP-MARKETPLACE-MATCHMAKING](../project_glossary.md#cap-marketplace-matchmaking) | CON-23A501C051 (Anonymity), CON-C22D030D21 (FTC Guidelines) |
| Beneficiary ([ACT-ADA6716160](../project_glossary.md#act-ada6716160)) | CAP-MARKETPLACE-MATCHMAKING, CAP-MERCHANT-NGO-OPERATIONS | CON-2788862587 (Data Sensitivity), CON-92F07E31B0 (Data Isolation) |
| Merchant ([ACT-AF904DCFF9](../project_glossary.md#act-af904dcff9)) | CAP-MERCHANT-NGO-OPERATIONS, [CAP-TRANSACTION-FINANCIAL-ENGINE](../project_glossary.md#cap-transaction-financial-engine) | CON-5D64EBC654 (Latency), CON-4820FAD5A9 (Data Retention) |
...
| Platform Administrator ([ACT-086A974D63](../project_glossary.md#act-086a974d63)) | CAP-COMPLIANCE-SECURITY-AUDIT, CAP-IDENTITY-ACCESS-MANAGEMENT | PCI-DSS Level 1, SOC2 Type II, CON-FD21121DD5 (Uptime) |
| Dispute Adjudicator ([ACT-7BA340FF76](../project_glossary.md#act-7ba340ff76)) | CAP-DISPUTE-RESOLUTION-CHARGEBACK-MANAGEMENT, CAP-FRAUD-DETECTION-FRAUD-PREVENTION-SCREENING | CON-B1DFEBEC8C (Quasi-Cash Regulations) |
...
### 2.5 Cross-Reference to Sibling Artifacts
...
### 3.1. PCI-DSS Level 1 Adherence and Payment Isolation
...
### 3.4. Financial Ledger Integrity and Auditability
...
### 3.6. NGO Operator Legal Entity Structure
...
## 5. Financial Operating Model for the Transaction & Financial Engine

This section defines the strategic operating model for the Transaction & Financial Engine, detailing the flow of funds, liability management, and fraud prevention mechanisms. It ensures alignment with the tripartite actor ecosystem (Donor, Beneficiary, Merchant) and NGO Operator governance, while strictly adhering to PCI-DSS Level 1 and SOC2 Type II compliance requirements.

### 5.1. Donor Funding & Credit Pool Management
...
### 5.2. Merchant Payout & Liability Management
...
### 5.4. Fraud Detection & Prevention Screening
...
### 1.4 NGO Operator (ACT-09E028AEB0)
...
*   **Core Responsibilities:**
    ...
    *   Initiate automated financial reconciliation against donor funds.
...
*   **Core Responsibilities:**
    ...
    *   Issue binding decisions on chargebacks and refund reversals.
*   **Compliance & Security Boundaries:**
    *   **Chargeback Management:** The platform must implement robust Dispute Resolution & Chargeback Management capabilities (CAP-DISPUTE-RESOLUTION-CHARGEBACK-MANAGEMENT) to handle financial reversals securely.

## 2. Capability Mapping & Operational Boundaries

This section maps the actor ecosystem to the platform's core capabilities, ensuring that every user journey is supported by a defined operational boundary.

### 2.1 Identity & Access Management (CAP-IDENTITY-ACCESS-MANAGEMENT)
...
*   **Scope:** Manages the credit pool, liquidity, and financial ledger mutations.
*   **Operational Boundary:**
    *   **Credit Pool Management:** Monitors Credit Pool Utilization Rate with automated alerts triggering when thresholds exceed 85% (CON-7031BE57B3). Unused emergency credits must automatically roll back to the central regional pool after 72 hours of expiration to prevent ledger stagnation.
    *   **Liquidity Health:** Tracks Donation-to-Redemption Velocity (DRV) to monitor liquidity health against the 14-day target (CON-F89C70071E).
    *   **Ledger Integrity:** Implements append-only cryptographic log auditing in Aurora PostgreSQL for all financial ledger mutations (CON-6061FCCA83).
    *   **Edge Cases:** Handles financial edge cases such as double-spending prevention and voided transactions (CON-61EC670500).

### 2.3 Merchant & NGO Operations (CAP-MERCHANT-NGO-OPERATIONS)
...
### 2.5 Fraud Detection & Fraud Prevention Screening (CAP-FRAUD-DETECTION-FRAUD-PREVENTION-SCREENING)
...
The platform will transition from a single-city MVP architecture to a resilient multi-tenant architecture optimized for 50,000 active users across three metro footprints (SF, NYC, Chicago).
