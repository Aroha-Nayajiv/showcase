...
- **KPI‑01 Form Completion Rate** – Target >90 % of submissions completed within 3 minutes (server logs).
- **KPI‑02 Encryption Validation** – 100 % of stored PHI fields pass AES‑256‑GCM decryption test weekly.
- **KPI‑03 Audit Log Coverage** – 100 % of read/write operations recorded, verified by log integrity checksum.
- **KPI‑04 Deployment Time** – Full stack up in ≤5 minutes on reference hardware, validated by automated script.
- **KPI‑05 System Availability** – ≥99.9 % uptime monitored by Prometheus alerts.
...
|---|---|---|---|---|
| RISK‑001 | Encryption key management failure | Medium | High | Deploy HashiCorp Vault (open-source) with automated rotation and audit logging; quarterly key rotation drills |
| RISK‑002 | Air‑gap deployment misconfiguration | Low | Medium | Provide offline image repository with SHA‑256 verification; step-by-step air-gap guide validated in test lab |
| RISK‑003 | Unauthorized access due to RBAC misconfiguration | Medium | High | Implement role-based policies reviewed against a security checklist; enforce PostgreSQL row-level security; MFA for admin/clinician |
| RISK‑004 | Audit log tampering | Low | High | Store logs in append-only WORM storage; compute hash chain per entry; verify integrity nightly |
| RISK‑005 | Performance degradation under load | Medium | Medium | Conduct load testing with Locust; ensure <200 ms response; enable connection pooling and caching 
