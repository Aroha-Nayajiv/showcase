...
- **RISK-005** – Open‑source component vulnerability discovered post‑deployment (Medium/Medium). *Mitigation*: Maintain vulnerability scanning pipeline; subscribe to CVE feeds; patch within 30 days of disclosure. *Acceptance*: Patch applied within SLA for all identified CVEs.
...
## 2. Scope Definition
**In‑Scope**:
- Web‑based structured intake form with field‑level encryption at rest (AES–1¶) and in transit (TLS 1.3) – see FR-001.
...
|----|----------|-------------|-------------------|
| COMP-HIPAA-001 | 45 CFR §164.312(a)(2)(iv) | Encrypt PHI at rest (AES–1¶) | Configuration scan confirming all PHI columns encrypted |
...
## 6. Business Vision and Objectives
...
- **FR-002**: Encrypt all form field data at rest using AES–1¶ and in transit using TLS 1.3. *Acceptance*: Independent security scan confirms encryption for every stored column and all network traffic.
...
## 11. Success Criteria
- **SC-001**: All patient data captured via the web form must be encrypted at rest (AES–1¶) and in transit (TLS 1.3). *Verification*: Audit of encryption configuration shows 100% coverage.
