...
#### 2.3 Field‑Level Encryption Example
sql
...
All PHI columns use **pgp_sym_encrypt** with a per‑record Data Encryption Key (DEK) that is itself encrypted by a master key stored in HashiCorp Vault.
...
#### 4.2 API Definitions (Intake Service)
...
#### 4.3 PDF Export Service Contract
...
### 5. Docker Compose Configuration (Production‑Ready)
yaml
version: "3.9"
services:
  auth-gateway:
...
### 6. Compliance & Traceability Matrix
...
### 8. Deployment Hardening & Operational Controls
