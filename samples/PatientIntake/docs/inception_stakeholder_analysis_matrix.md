...
The solution adopts a fully open‑source stack: an HTML5/JavaScript front‑end built on React, a Flask‑based API layer (Python 3.11), and PostgreSQL 15 for data persistence. TLS termination is performed by an open‑source reverse proxy (Traefik) configured for strict cipher suites (AES‑256‑GCM). PDF generation uses the open‑source library WeasyPrint, and watermarking is applied via PDF‑tk. All containers are orchestrated by Docker Compose version 2.4, with explicit network isolation to satisfy the air‑gap requirement. The architecture diagram (referenced as ARCH‑001) shows three logical zones: Presentation, Business Logic, and Data Store, each protected by firewall rules enforced at the container‑network level.
